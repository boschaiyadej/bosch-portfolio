...
const ExCard: React.FC<ExCardProps> = ({ icon, year, title, desc }) => {
  return (
    <div className="relative pl-[3.125rem] pb-2 sm:pb-12 last:pb-0 border-l-[2px] border-accent">
      ...
      <span className="sm:text-base text-sm hover:text-accent">{year}</span>
      <h3 className="sm:text-base text-sm font-bold hover:text-accent">
        {title}
      </h3>
      <p className="sm:text-base text-sm hover:text-accent">{desc}</p>
    </div>
  );
};
